...
export async function POST(req) {
  ...
  try {
    ...
    if (contentType === 'application/json') {
      const { source_image, face_image, index, webhook } = await req.json();
      console.log('JSON 요청 처리 중');
      const response = await axios.post(
        'https://aifaceswap.io/api/aifaceswap/v1/multi_faceswap',
        { source_image, face_image, index, webhook },
        {
          headers: {
      ...
      // index 처리
      apiFormData.append('index', index);
      
      // webhook 처리 (선택적)
      if (webhook) apiFormData.append('webhook', webhook);
